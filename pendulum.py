...
class Pendulum:
    def __init__(self, length, angle, angular_velocity=0.0, bob_mass=1.0, scale_force_xy=(-0.1, -0.1), frequency_gain=1.0):
        self.length = length
        self.angle = angle
        self.angular_velocity = angular_velocity
        self.bob_mass = bob_mass
        self.scale_force_xy = scale_force_xy
        self.frequency_gain = frequency_gain

    def update(self, dt):
        gravity = 9.81  # gravitational constant
        angular_acceleration = (-gravity / (self.length*scale_factor)) * math.sin(self.angle) * self.frequency_gain
